...
u8 RTC_Init(void)
{
	...
	{	 			
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2021,1,15,12,35,0);  //设置时间	
...
#if 1		
		  SysMemoryParaVar.hour = calendar.hour;
		  SysMemoryParaVar.day = calendar.w_date;
		  SysMemoryParaVar.month = calendar.w_month;
	  	  SysMemoryParaVar.year = calendar.w_year;
		  SysMemoryParaVar.min = calendar.min;
		  SysMemoryParaVar.sec = calendar.sec;
 #endif
