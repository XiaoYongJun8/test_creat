...
void  OSInit (void)
{
    OSInitHookBegin();   //禁用定时器                            /* 调用端口特点的初始化代码Call port specific initialization code   */

    OS_InitMisc();                                               /* 初始化各种变量Initialize miscellaneous variables       */

    OS_InitRdyList();                                            /* 初始化就绪列表Initialize the Ready List                */

    OS_InitTCBList();                                            /* 初始化TCB的空闲列表Initialize the free list of OS_TCBs      */
...
static  void  OS_InitMisc (void)
{
    ...
    OSIntNesting  = 0;                                     /*清除中断嵌套计数器 Clear the interrupt nesting counter      */
